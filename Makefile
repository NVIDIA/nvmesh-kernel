...
# NVMEIB_COMPILATION_DATE is only used by clnt/block/um_integration/test.c,
# which has its own Makefile. Do not add it to the shared cflags - it would
# change every day and force a full rebuild of every object.
...
# Build-info macros (COMMIT_ID, BRANCH_NAME, NVMESH_VERSION, NVMESH_RELEASE,
# BUILD_DISTRO, BUILD_NUMBER) used to live in the global kernel cflags, which
# forced every .o to rebuild on every commit bump because kbuild's if_changed
# detects the changed command line.  They now live in a cmp-swap-generated
# header included explicitly by the ~10 files that actually consume them; see
# GEN_BUILD_INFO below and autogen/common/nvmeib_build_info.h.
#
# UTILSFLAGS keeps -DCOMMIT_ID because the userspace utility Makefiles read it
# from the environment and do not include the header.
ifneq ($(COMMIT_ID),)
...
# lz4 is a vendored library driven by a sub-make whose targets are .PHONY,
...
# Build-info header: cmp-swap so mtime is only bumped when content actually
# changes.  The ~10 kernel TUs that reference COMMIT_ID / NVMESH_VERSION /
# etc. include this header explicitly; files that do not include it are
# unaffected by commit bumps, so kbuild's if_changed does not re-cook them.
BUILD_INFO_H := $(AUTOGEN_DIR)/common/nvmeib_build_info.h
GEN_BUILD_INFO = \
	mkdir -p $(AUTOGEN_DIR)/common; \
	printf '%s\n' \
	    '/* Auto-generated by top-level Makefile; do not edit. */' \
	    '\#ifndef NVMEIB_BUILD_INFO_H' \
	    '\#define NVMEIB_BUILD_INFO_H' \
	    '\#ifndef COMMIT_ID' \
	    '\#define COMMIT_ID 0x$(COMMIT_ID)' \
	    '\#endif' \
	    '\#ifndef BRANCH_NAME' \
	    '\#define BRANCH_NAME "$(BRANCH_NAME)"' \
	    '\#endif' \
	    '\#ifndef NVMESH_VERSION' \
	    '\#define NVMESH_VERSION $(VERSION)' \
	    '\#endif' \
	    '\#ifndef NVMESH_RELEASE' \
	    '\#define NVMESH_RELEASE $(RELEASE)' \
	    '\#endif' \
	    '\#ifndef BUILD_DISTRO' \
	    '\#define BUILD_DISTRO $(DISTRO_TAG)' \
	    '\#endif' \
	    '\#ifndef BUILD_NUMBER' \
	    '\#define BUILD_NUMBER $(PACKAGE_BUILD_NUMBER)' \
	    '\#endif' \
	    '\#endif' \
	    > $(BUILD_INFO_H).tmp; \
