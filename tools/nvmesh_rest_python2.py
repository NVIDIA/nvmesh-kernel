...
def find_mgmt_server_in_fp(fp):
    ...
    raise Exception("No mgmt found in {0}".format(NVMESH_CONFIG))
...
def detach_cb(args, session):
    ...
    if args.all:
        reply = session.get('/clients/{0}'.format(client), verify=False)
