...
// Fast AVX implementation. Used in userspace / Kernel
...
/*    Reverse : TRUE.                                            */
/*****************************************************************/
static const u32 __attribute__((aligned(1024))) crc_table[256] = {
...
static u32 __attribute__((unused)) __crc32c_unopt_table(u32 crc, const void *buf, unsigned int length) {
	while (length-- > 0) {
		crc = (crc>>8) ^ crc_table[(crc ^ (*(u8*)buf++)) & 0xFF];
	}
	return crc;
}

static u32 __attribute__((unused)) __crc32c_unopt_mask(u32 crc, const void *buf, unsigned int length) {
	...
	u32 crc32c(u32 crc, const void *buf, unsigned int length) {	// All implementations return identical results
		...
		#elif 0
			return __crc32c_unopt_mask(crc, buf, length);
		#else
			return __calculate_crc32c_avx(crc, buf, length);
		#endif
	}
#endif // !KS_HAS_CRC32C

#endif // KR_INCS_CRC32_IMPL_H
