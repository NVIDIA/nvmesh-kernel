...
void nvmeibc_jam_simu_process_entry_event(struct nvmeibc_disk *disk, int idx, enum nvmeibc_jam_jidx_event event){
	int rv = jidx_event(disk, idx, event);
	BUG_ON(rv != 0);
}
...
static int jentry_erase(struct nvmeibc_disk *disk, struct nvmeibc_jam_jidx *jidx)
{
	...
	rv = icore_ops->execute_gen(icore_ops, &disk->base, gen_cmd);
...
int nvmeibc_jam_jentry_erase_comp(struct nvmeibc_disk_gen_cmd *gen_cmd)
{
	...
	icore_ops->cb_called_cmd(icore_ops, &disk->base, &gen_cmd->disk_cmd);
...
int nvmeibc_jam_abandon_lba(struct nvmeibc_idisk *idisk, u64 jlba, u8 *gen_id)
{
	...
	if (!icore_ops->jam_get(icore_ops, &disk->base)) {
		...
		icore_ops->jam_put(icore_ops, &disk->base);
	}
	else {
		rv = -1;
...
void pd_rollback_partial_allocation(struct nvmeibc_jam_pending_req *jreq)
{
	...
	while (--i >= 0) {
		if (!icore_ops->jam_get(icore_ops, &sorted[i].disk->base)) {
			lba = res_jlbas[sorted[i].orig_pos];
			jlba_put_unused(sorted[i].disk, lba);
			icore_ops->jam_put(icore_ops, &sorted[i].disk->base);
		}
		else
