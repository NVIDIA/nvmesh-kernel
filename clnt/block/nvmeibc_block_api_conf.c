...
int nvmeibc_block_reconf(struct nvmeibc_volume_conf *conf, struct nvmeibc_volume *volume)
{
	...
	rv = nvmeibc_topology_update_configuration(&next_conf, conf, version, true, nvmeibc_volume_targets_base(&volume->targets));
	if (rv < 0) {
...
int nvmeibc_block___conf(struct nvmeibc_volume_conf *conf, struct nvmeibc_volume *volume, struct nvmeibc_block_device *dev)
{
	...
	rv = nvmeibc_topology_update_configuration(&dev->topologies, conf, version, false, nvmeibc_volume_targets_base(&volume->targets));
	if (rv < 0)
