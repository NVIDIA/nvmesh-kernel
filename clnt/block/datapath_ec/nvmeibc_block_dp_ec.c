...
static void __calc_and_apply_io_dbits_action(struct nvmeibc_block_command *rldr)
{
	...
	nvmeibc_dbits_tx_init_by_bmp(&dbmap, &pr->calculated_data.topo_traits, turn_on_dbit_bmp, turn_off_dbit_bmp, 0 /* turn_on_conv_bmp */);
...
{
	const struct nvmeibc_disk_client_journal *jrnl = jrnl_cmd->ds->disk->ops.get_journal(jrnl_cmd->ds->disk);
...
static void __prepare_data_wr_cmds_md_with_journal(struct nvmeibc_block_command *rldr)
{
	...
	for_each_column_for_each_snake_while(mssa, mssa->bio_map, map_i, col, row, (ci++ < (mssa->n_writes / 2))) {
		...
		const struct nvmeibc_disk_client_journal *jrnl = jrnl_cmd->ds->disk->ops.get_journal(jrnl_cmd->ds->disk);
		const struct nvmeibc_disk_client_journal *io_jrnl = io_cmd->ds->disk->ops.get_journal(io_cmd->ds->disk);
