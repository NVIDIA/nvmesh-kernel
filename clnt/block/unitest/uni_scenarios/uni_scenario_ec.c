...
static int __get_jour_size(struct nvmeibc_disk_segment *seg) {
	return seg->disk->jour.rng_nlba/nvmeibc_jentry_num_blocks;
}
static void* __get_seg_journal_md_ptr(struct nvmeibc_disk_segment *seg) { return ramDiskSimulator_get_metadataptr(&serverOf(seg->disk)->ramDisk, seg->disk->jour.rng_slba); }
