...
ssize_t nvmeibc_disk_print_info(struct nvmeibc_disk *disk, char *buf, int len){ /* Print info of disk in JSON format */
	...
	ssize_t count = 0;
	BUF_ADD("{\"name\":\"%s\"}\n", ((disk)->base.ops.get_name(&((disk))->base)));
	return count;
	#undef BUF_ADD
}
...
static int call_discover(struct nvmeibc_disk *disk){
	struct nvmeibc_disk_client_journal *jour = ((disk)->base.ops.get_journal_mut(&((disk))->base));
	void *jrange_handle;
...
static ssize_t stats_fill_buf(void *priv, char *buf, size_t len) {
	...
	count += jops->data_str(                      buf+count, len-count, "uuid", ((disk)->base.ops.get_name(&((disk))->base)), !JSON_LAST_ELEM, indent);
...
static int _init_dirty_bits_mem(struct nvmeibc_disk *disk) {
	...
	_NT(trace_alloc_dirty_bits_mem, "allocating ec dirty bit pages for @DISK_NAME", ((disk)->base.ops.get_name(&((disk))->base)));
...
int nvmeibc_disk_create(const struct nvmeibc_cinst_params_core *p, struct nvmeibc_disk_id *disk_id, struct list_head *arnics, int num_ranges, const char *node_id){
	...
	snprintf(disk->full_name, sizeof(disk->full_name), "%.*s-%.*s", NVMEIB_HOST_NAME_LEN, utsname()->nodename, (int)sizeof(disk->name), ((disk)->base.ops.get_name(&((disk))->base)));
	disk->disk_host[0] = '?';
	disk->access_local = true;							// As if this disk is local to client. Todo: Control it better for elect/local read optimization tests
	disk->stats = nvmeib_io_stats_create_traced(((disk)->base.ops.get_name(&((disk))->base)), VERB_RW_T_RECOV_BITMASK, NVMEIBC_SECTOR_SIZE);
	disk->proc_ent_stats =      nvmeib_public_proc_create(((disk)->base.ops.get_name(&((disk))->base)), nvmeibc_get_proc_dir_disks(p), stats_fill_buf, NULL, disk);
	disk->proc_ent_stats_json = nvmeib_public_proc_create(((disk)->base.ops.get_name(&((disk))->base)), nvmeibc_get_proc_dir_disks(p), stats_fill_buf, NULL, disk);
	...
	_NT(trace_simu_disk_nvmeibc_disk_create, "Disk @DISK_NAME is connected", ((disk)->base.ops.get_name(&((disk))->base)));
	return rv;
}
...
bool nvmeibc_disk_remove(struct nvmeibc_disk_id *disk_id, bool block){
	...
	if (is_empty) {
		...
		_NT(trace_1_simu_disk_nvmeibc_disk_remove, "calling disk release from nvmeibc_disk_remove disk:@DISK_NAME", ((disk)->base.ops.get_name(&((disk))->base)));
		disk->detached = true;
...
static void complete_disk_pause(void *v) {
	...
	if ((rv = atomic_dec_return(&disk->n_volumes_paused)) <= 0) {
		if (rv < 0)
			_NW_dmesg(warn_simu_disk_complete_disk_pause, "Block module over triggered the disk @DISK_NAME (@DISK) pause completion (@RV)", ((disk)->base.ops.get_name(&((disk))->base)), disk, rv);
...
void nvmeibc_disk_pause(struct nvmeibc_disk *disk)
{
	...
	BUG_ON(((disk)->base.ops.read_cont_preventors(&((disk))->base)) < 0);
		...
		if (disk_id->volume) {
			...
			if (is_ready>0) {
				if (nvmeibc_block_cont(disk_id->volume->block_dev, disk)) {
					_NE_dmesg(error_simu_disk_cont_disk_io, "Failed to start IO on disk @DISK_NAME for volume @DEV_NAME_FULL", ((disk)->base.ops.get_name(&((disk))->base)), disk_id->volume->full_name);
					--rv;
				}
			} else if (!is_ready) {
				/* No need to reschedule, becuase bdev tests status of disk */
...
bool nvmeibc_pd_wait_for_pause_completion_debug(struct nvmeibc_disk *disk){
	...
	if (last_transfers!=-1)
		_Emerg("-Transferring = %d, Preventors=%d pausing=%d, should_pause=%d\n", disk->percpu->in_transfers, disk->percpu->pause_preventers, disk->pausing, ((disk)->base.ops.should_pause(&((disk))->base)));
...
int nvmeibc_disk_release(struct nvmeibc_disk *disk)
{
	struct nvmeibc_disk_client_journal *jour = ((disk)->base.ops.get_journal_mut(&((disk))->base));
	int dying, rv = 0;
	_NT(trace_simu_disk_nvmeibc_disk_release, "Starting exceution of disk_release @DISK_NAME", ((disk)->base.ops.get_name(&((disk))->base)));
	...
	if ((dying = atomic_inc_return(&disk->dying)) > 1) {
		_NT(trace_2_simu_disk_nvmeibc_disk_release, "Release in process for disk @DISK_NAME (@DYING)", ((disk)->base.ops.get_name(&((disk))->base)), dying);
		goto out;
	}
...
int nvmeibc_disk_start_release(struct nvmeibc_disk *disk, enum nvmeibc_disk_release_reason reason)
{
	...
	_NT(info_simu_disk_start_release, "disk @DISK_NAME, attempt start-release with reason: @DISK_RELEASE_OP",	((disk)->base.ops.get_name(&((disk))->base)), reason);
...
retry:
	if ((restart_called = atomic_inc_return(&disk->restart_called)) == 1) {
		...
		if ((rv = nvmeibc_disk_add_work(disk, &rwork->work)) < 0) {
			...
			_NT(trace_simu_disk_start_release_2, "Initiate disk @DISK_NAME release", ((disk)->base.ops.get_name(&((disk))->base)));
	}
	else {
...
int nvmeibc_disk_set_next_config(struct nvmeibc_disk_id *disk_id, const char *node_id){
	...
	_NT(trace_simu_disk_nvmeibc_disk_set_next_config, "New config node id @NODE_ID_STR will replace the existing one disk @DISK_NAME", node_id, ((disk)->base.ops.get_name(&((disk))->base)));
...
static bool __does_node_and_nic_exist(struct nvmeibc_disk *disk) {  // Checking if toma simulator disk is accessed correctly
	...
	if (!arnic || strcmp(srvr->hardware->node_id, arnic->node_id)) { // Either disk has no arnics or disk resides on a server that is not the same as the arnic it uses
		_NT(trace_simu_disk_does_node_and_nic_exist, "disk @DISK_NAME: Ignoring msg, due to arnic mismatch server node=@NODE, arnic node=@NODE", ((disk)->base.ops.get_name(&((disk))->base)), srvr->hardware->node_id, (arnic ? arnic->node_id : "NULL"));
		rv = false;
	} else if (strcmp(srvr->hardware->disk_name, ((disk)->base.ops.get_name(&((disk))->base)))) {   // Disk resides on a different server than this toma
		_NT(trace_1_simu_disk_does_node_and_nic_exist, "disk @DISK_NAME: Ignoring msg, due to server mismatch (disk @DISK_NAME)", ((disk)->base.ops.get_name(&((disk))->base)), srvr->hardware->disk_name);
		rv = false;
	}
	spin_unlock(&disk->disk_conf_spinlock);
	return rv;
