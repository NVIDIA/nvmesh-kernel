	...
	for (lock_ind=lock_ind_first; lock_ind<=lock_ind_last; lock_ind++) {
		...
		for (li = 0; li < (u64)rlmap.n_locks; li++) {						// verify each member has the lock that the lock map says it needs
			...
			if (!nvmeib_lockid_are_purified_eq(lock_val, r1->lid)) {
				BUG_ON(nvmeibc_disk_from_base(seg->disk)->pausing_no_transfers != true);	// BEWARE: a quick pause/cont on one member while the other is still executing the IO from before the pause, might not catch the pause & fail !!!
				_NI_dmesg(t_01vlbi, "member @SI (disk @DISK_NAME) of r1 @R1 is paused - cannot verify lock state", si, seg->disk->ops.get_name(seg->disk), r1);
			}
