...
#define LARRAY_INIT(_a) \
	do { \
		int CONCAT(i, __LINE__); \
		memset((_a).a, 0, sizeof((_a).a)); \
		BUILD_BUG_ON(sizeof((_a).a[0]) < sizeof(int));\
		for (CONCAT(i, __LINE__) = 0; \
				CONCAT(i, __LINE__) < ARRAY_SIZE((_a).a) - 1; \
				++CONCAT(i, __LINE__)) \
			*((int *)&((_a).a[CONCAT(i, __LINE__)])) = \
				CONCAT(i, __LINE__) + 1; \
...
#define LARRAY_GET(_a) \
	({\
		int CONCAT(index, __LINE__) = (_a).next; \
		if (likely((_a).next != -1)) (_a).next = \
			*((int *)&((_a).a[(_a).next])); \
		CONCAT(index, __LINE__); \
	})
