...
TMP=$(mktemp -d -p `pwd`)
cd $TMP
sudo dnf download kernel-${KERNEL_NO_X86} --disablerepo="*" --enablerepo="rhel-*-for-x86_64-baseos-source-rpms" --source
...
SECRETS="drivers/infiniband/hw/mlx4/mlx4_ib.h drivers/infiniband/hw/mlx5/mlx5_ib.h drivers/infiniband/hw/mlx5/srq.h"
cd -
mkdir -p $KERNEL
...
for sec in $SECRETS; do
    ...
    mv ${KERNEL_SRCS_PATH}/$sec $tgt_dir
done

echo "Done. secrets added to ${KERNEL}"