...
bool nvmeibt_seg_active_final_free_if_not_in_use(struct nvmeibt_seg_active *seg_active)
{
	...
	if (seg_active->ref_count) {
		...
		NNVMEIBT_BM_FREE(fhyy734, seg_active->persistent_metadata);
		{
			...
			if (pt_err) {
				N_Ef(ry876bq, "Failed to destroy stale locks mutex err=@INT (@STR)", pt_err, strerror(pt_err));
			}
		}
...
void nvmeibt_seg_active_free_mem_and_processes(struct nvmeibt_seg_active *seg_active)
{
	...
	NVMEIB_HASH_FOREACH(stale_lock, seg_active->stale_locks_hash_by_seg_blkset_no) {
		...
		--(stale_lock->reg_ctx->n_stale_locks);
		NNVMEIBT_BM_FREE(bnsj29k, stale_lock);
	}
	NVMEIB_HASH_TBL_FREE(5isdkom, seg_active->stale_locks_hash_by_seg_blkset_no);
	//
	unlock_stale_locks_hash(seg_active);
	NVMEIB_HASH_FOREACH(reg_ctx, seg_active->longing_registrants_hash_by_handle) {
		nvmeibt_register_terminate_reg_ctx(reg_ctx, 1, NVMEIBT_REGISTER_REGISTRANT_TYPE_LONGING, 0);
	}
	...
	NVMEIB_HASH_FOREACH(reg_ctx, seg_active->active_registrants_hash_by_handle) {
		nvmeibt_register_terminate_reg_ctx(reg_ctx, 1, NVMEIBT_REGISTER_REGISTRANT_TYPE_ACTIVE, 0);
	}
	...
	NVMEIB_HASH_FOREACH(reg_ctx, seg_active->stale_registrants_hash_by_purified_lockid) {
		nvmeibt_register_terminate_reg_ctx(reg_ctx, 1, NVMEIBT_REGISTER_REGISTRANT_TYPE_STALE, 0);
	}
	...
	NVMEIB_HASH_TBL_FREE(vuilso5, seg_active->awaited_lockids_hash_by_lockid);
	//
													 ...
													 struct nvmeibt_seg_active_metadata_ctrl *metadata_ctrl_fr_persist)
{
...

	{
		...
		if (pt_err) {
			N_Ef(ry876ha, "Failed to create stale locks mutex err=@INT (@STR)", pt_err, strerror(pt_err));
			nvmeibt_abort(ES_FATAL);
		}
	}
...
static void remove_stale_lock_from_seg_stale_locks_hash(struct stale_lock_ctx *stale_lock)
{
	...
	if (--(stale_reg_ctx->n_stale_locks) == 0 && !nvmeibt_register_is_processing_registrant_removal(stale_reg_ctx)) {
		nvmeibt_register_terminate_reg_ctx(stale_reg_ctx, 0, NVMEIBT_REGISTER_REGISTRANT_TYPE_STALE, 0);
	}
	NFOUT;
}
	...
	struct nvmeibt_seg_active *seg_active, unsigned long long seg_blkset_no, union nvmeib_lock_id lockid)
{
	...
	NFIN;
	lock_stale_locks_hash(seg_active);
	stale_lock = nvmeib_hash_search_uint64_t(seg_active->stale_locks_hash_by_seg_blkset_no, seg_blkset_no);
	if (stale_lock) {
	...
	struct nvmeibt_registrant_ctx *reg_ctx, unsigned long long seg_blkset_no, const union nvmeib_lock_id existing_lock_id)
{
	...
	existing_stale_lock = nvmeib_hash_search_uint64_t(seg_active->stale_locks_hash_by_seg_blkset_no, seg_blkset_no);
	if (existing_stale_lock) {
		if (existing_stale_lock->reg_ctx == reg_ctx) {
			N_Wf(xtvsjw9, "Stale entry already exists " STALE_BLKSET_FMT " client_id=@MY_HOSTNAME",
				existing_stale_lock->seg_blkset_no, nvmeibt_seg_active_UUID_8(seg_active),
				nvmeib_lockid_purify(existing_stale_lock->reg_ctx->reg_lock_id),
				existing_stale_lock->reg_ctx->client->net.host_name);
			goto out;
		} else {
			if (existing_lock_id.bits.is_read) {
	...
	stale_lock_to_restore = NNVMEIBT_BM_ALLOC(hf7i30w, sizeof(*stale_lock_to_restore));
	stale_lock_to_restore->seg_blkset_no = seg_blkset_no;
	stale_lock_to_restore->reg_ctx = reg_ctx;
	stale_lock_to_restore->lockid_that_was_left_behind = existing_lock_id;
	reg_ctx->n_stale_locks++;
	nvmeib_hash_add_uint64_t(seg_active->stale_locks_hash_by_seg_blkset_no, seg_blkset_no, stale_lock_to_restore);
	N_Tf(ts6tslh, "Adding " STALE_BLKSET_FMT, seg_blkset_no, nvmeibt_seg_active_UUID_8(seg_active), nvmeib_lockid_purify(reg_ctx->reg_lock_id));
out:
	unlock_stale_locks_hash(seg_active);
	NFOUT;
...
int nvmeibt_recovery_execute_stale_and_txid_rebuilds(struct nvmeibt_seg_active **seg_active_arr, int n_seg_active)
{
	int								n_launched_tasks = 0;
	...
	for (n_seg_active-- ; n_seg_active >= 0; n_seg_active--) {
		...
		// The following is inside the loop since N_RUNNING_TASKS might change.
		if (NVMEIBT_GLOBAL_GET_N_TASKS_COUNTER(n_running_stale_rebuild) + NVMEIBT_GLOBAL_GET_N_TASKS_COUNTER(n_running_txid_rebuild) >=
			(int)nvmeibt_recovery_get_max_n_simultaneous_stale_and_txid_rebuild()) {
			N_Tf(guy690f, "Skipping, n_running_stale_rebuild=@INT n_running_txid_rebuild=@INT",
				 NVMEIBT_GLOBAL_GET_N_TASKS_COUNTER(n_running_stale_rebuild), NVMEIBT_GLOBAL_GET_N_TASKS_COUNTER(n_running_txid_rebuild));
			goto out;
		}
		// Run recovery thread per this PRAID segment, that resides on this node, and is owner_recoverer
		if (nvmeibt_disk_segment_is_de_facto_owner(nvmeibt_seg_active_get_active_seg_topo(seg_active))) {
			if (nvmeibt_seg_active_is_stale_rebuild_required(seg_active)) {
				...
				if (IS_RECOVERY_TASK_RUNNING(seg_active, stale_rebuild)) {
					n_launched_tasks++;
					nvmeibt_global_get_global()->n_pending_stale_rebuild--;
				}
			} else {
				nvmeibt_seg_active_txid_rebuild(seg_active);
				if (IS_RECOVERY_TASK_RUNNING(seg_active, txid_rebuild)) {
					n_launched_tasks++;
					nvmeibt_global_get_global()->n_pending_txid_rebuild--;
				}
			}
		} else {
				...
				enum NVMEIBT_RECOVERY_STATUS recovery_status)
{
	...
	if (recovery_status == NVMEIBT_RECOVERY_STATUS_SUCCESS) {
		...
		seg_active->cold_recovery_ctx.praid_version = 0;
		if (nvmeibt_seg_active_mark_cold_recovery_required_if_needed(seg_active))
			NVMEIBT_SEG_ACTIVE_MARK_ARE_POST_UPDATE_ACTIONS_REQUIRED(behbs71, seg_active);
	}
	NFOUT;
}
...
void nvmeibt_seg_active_handle_post_update_actions(struct nvmeibt_seg_active *seg_active)
{
	...
	if (seg_active->dirty_rebuild_ctx.praid_version < nvmeibt_seg_active_get_active_praid_version_major(seg_active))
		nvmeibt_seg_active_mark_dirty_rebuild_required_if_needed(seg_active);
	if (seg_active->cold_recovery_ctx.praid_version < nvmeibt_seg_active_get_active_praid_version_major(seg_active))
		nvmeibt_seg_active_mark_cold_recovery_required_if_needed(seg_active);
...
static void launch_seg_active_zero_task(struct nvmeibt_seg_active *seg_active, uint64_t n_4Kblk_zeroed, struct netlink_io_context *nl_ctx, int fd, int last_CHANGE_no)
{
	...
	if (nvmeibt_local_disk_add_work_with_ldisk_last_CHANGE_no(its_local_disk, &(seg_zero_task->wq_entry)) != 0) {
...
#if 0
test_if_specific_recovery_needed(struct nvmeibt_seg_active *seg_active)
{
	if (func) {
		if (array_idx < max_array_idx)
			segs_array[array_idx++] = seg_active;
		return 1;
	} else {
		return 0;
	}
...
void nvmeibt_seg_active_scan_all(void)
{
	...
	struct nvmeibt_seg_active			**cold_recovery_arr;
	struct nvmeibt_seg_active			**dirty_rebuild_raid1_arr;
	struct nvmeibt_seg_active			**dirty_rebuild_ec_arr;
	struct nvmeibt_seg_active			**stale_and_txid_rebuild_arr;
	int									cold_recovery_arr_idx = 0;
	int									dirty_rebuild_raid1_arr_idx = 0;
	int									dirty_rebuild_ec_arr_idx = 0;
	int									stale_and_txid_rebuild_arr_idx = 0;
	int									n_seg_active = 0;
	...
	dirty_rebuild_ec_arr = dirty_rebuild_raid1_arr + n_seg_active;
	stale_and_txid_rebuild_arr = dirty_rebuild_ec_arr + n_seg_active;
	...
	NVMEIB_HASH_FOREACH(local_disk, global_params->nvmesh_local_disks_hash_by_ldisk_id_str) {
		...
		NVMEIB_HASH_FOREACH(seg_active, local_disk->seg_active_hash_by_uuid) {
			...
			if (nvmeibt_seg_active_is_cold_recovery_required(seg_active)) {
				cold_recovery_arr[cold_recovery_arr_idx++] = seg_active;
				global_params->n_pending_cold_recovery++;
			}
			if (nvmeibt_seg_active_is_dirty_rebuild_required(seg_active)) {
				if (nvmeibt_seg_active_is_EC(seg_active))
					dirty_rebuild_ec_arr[dirty_rebuild_ec_arr_idx++] = seg_active;
				else
					dirty_rebuild_raid1_arr[dirty_rebuild_raid1_arr_idx++] = seg_active;
				global_params->n_pending_dirty_rebuild++;
			}
			if (nvmeibt_seg_active_is_stale_rebuild_required(seg_active) ||
				nvmeibt_seg_active_is_txid_rebuild_required(seg_active)) {
				stale_and_txid_rebuild_arr[stale_and_txid_rebuild_arr_idx++] = seg_active;
				if (nvmeibt_seg_active_is_stale_rebuild_required(seg_active))
					global_params->n_pending_stale_rebuild++;
				if (nvmeibt_seg_active_is_txid_rebuild_required(seg_active))
					global_params->n_pending_txid_rebuild++;
			}

			if (nvmeibt_seg_active_is_JGC_rebuild_required(seg_active)) {
				nvmeibt_seg_active_JGC_rebuild(seg_active);
				if (!IS_RECOVERY_TASK_RUNNING(seg_active, JGC_rebuild))
					global_params->n_pending_JGC_rebuild++;
			}
		}
	}

	global_params->n_pending_cold_recovery -= nvmeibt_recovery_execute_cold_recoveries(cold_recovery_arr, cold_recovery_arr_idx);
	global_params->n_pending_dirty_rebuild -= nvmeibt_recovery_execute_dirty_rebuilds(dirty_rebuild_raid1_arr,
																					  dirty_rebuild_raid1_arr_idx,
																					  nvmeibt_recovery_get_max_n_simultaneous_dirty_rebuild() * 2);
	global_params->n_pending_dirty_rebuild -= nvmeibt_recovery_execute_dirty_rebuilds(dirty_rebuild_ec_arr,
																					  dirty_rebuild_ec_arr_idx,
																					  nvmeibt_recovery_get_max_n_simultaneous_dirty_rebuild());
	nvmeibt_recovery_execute_stale_and_txid_rebuilds(stale_and_txid_rebuild_arr, stale_and_txid_rebuild_arr_idx);

	N_Tf(u878un6, "rebuild counters: cold(p=@INT r=@INT) dirty(p=@INT r=@INT) stale(p=@INT r=@INT) txid(p=@INT r=@INT) JGC(p=@INT r=@INT)",
		 NVMEIBT_GLOBAL_GET_N_TASKS_COUNTER(n_pending_cold_recovery), NVMEIBT_GLOBAL_GET_N_TASKS_COUNTER(n_running_cold_recovery),
		 NVMEIBT_GLOBAL_GET_N_TASKS_COUNTER(n_pending_dirty_rebuild), NVMEIBT_GLOBAL_GET_N_TASKS_COUNTER(n_running_dirty_rebuild),
		 NVMEIBT_GLOBAL_GET_N_TASKS_COUNTER(n_pending_stale_rebuild), NVMEIBT_GLOBAL_GET_N_TASKS_COUNTER(n_running_stale_rebuild),
		 NVMEIBT_GLOBAL_GET_N_TASKS_COUNTER(n_pending_txid_rebuild), NVMEIBT_GLOBAL_GET_N_TASKS_COUNTER(n_running_txid_rebuild),
		 NVMEIBT_GLOBAL_GET_N_TASKS_COUNTER(n_pending_JGC_rebuild), NVMEIBT_GLOBAL_GET_N_TASKS_COUNTER(n_running_JGC_rebuild));

	NNVMEIBT_BM_FREE(ru82nkk, cold_recovery_arr);
