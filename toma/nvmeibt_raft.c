...
static bool						raft_is_incremental_wire_buf_enabled = false;		// Disbled, as volume deletion does not work
								  ...
								  int old_len, int upd_len)
{
	...
	if (nvmeibt_tlv_get_type(old_wire_ctx) != TLV_TYPE_TOPO_COMPLETE) {
		N_Ef(wer923k, "Old topo is @INT8_TD not complete type. Incremental topo update requires old complete topo to merge with", nvmeibt_tlv_get_type(old_wire_ctx));
		return -1;
	}
	...
	if (dst_data_ptr) {
		*dst_wire_ctx = *old_wire_ctx;
		...
		memcpy(output_header, new_header, sizeof(*new_header));
		*dst_data_ptr += sizeof(*output_header);
	}

	NVMEIB_HASH_FOREACH(praid_obj, nvmeibt_global_get_global()->praids_hash_by_uuid) {
		praid_obj->praid_follower.is_serialized_in_incremental_merge = false;
	}

	// 1. Process all new praids from wire buffer
	n_praids_in_result = 0;
	for (i = 0; i < new_serialized_header.praids_num; i++) {
		...
		praid_obj = nvmeibt_praid_get_praid_by_id(&new_serialized_praid.uuid);
		if (praid_obj) {
			old_committed_topo = &(praid_obj->praid_follower.committed_praid_lot.topo_ctx);
			old_topo_idx_updated = old_committed_topo->topo_idx_updated;
			praid_obj->praid_follower.is_serialized_in_incremental_merge = true; // Flag early — praid_obj must not be accessed in the else branch below
		} else {
			if (nvmeibt_praid_is_client_sync_cmd_delete(new_serialized_praid.registrants_sync_cmd)) {
				old_topo_idx_updated = -2; // praid is deleted. Force incremental praid to be serialized (which also indicates deletion).
			} else {
		...
		if (old_topo_idx_updated < new_serialized_praid.topo_idx_updated) {
			...
			if (dst_data_ptr) {
				...
				*dst_data_ptr += new_praid_total_size;
			}
			total_size += new_praid_total_size;
		} else {
			// Old is same/newer: serialize from hash praid committed_praid_lot
			int										n_segs = 0;
			int										old_praid_size = 0;

			n_segs = XDLIST_N_ELEMNTS(&(praid_obj->praid_follower.committed_praid_lot.all_seg_lot_list));
			old_praid_size = sizeof(struct nvmeibt_praid_serialized_topo) + n_segs * sizeof(struct nvmeibt_serialized_seg_leader_topo);

			if (dst_data_ptr) {
				nvmeibt_serialize_praid_lot_topo_to_wire(&(praid_obj->praid_follower.committed_praid_lot),
				nvmeibt_praid_UUID(praid_obj),
				(struct nvmeibt_praid_serialized_topo *)*dst_data_ptr,
				(void *)(*dst_data_ptr + sizeof(struct nvmeibt_praid_serialized_topo)));
				*dst_data_ptr += old_praid_size;
			}
			total_size += old_praid_size;
		}
		n_praids_in_result++;
	...
	NVMEIB_HASH_FOREACH(praid_obj, nvmeibt_global_get_global()->praids_hash_by_uuid) {
		if (!praid_obj->praid_follower.is_serialized_in_incremental_merge) {
			int										old_praid_size = 0;
			int8_t									old_n_segs = 0;

			old_committed_topo = &(praid_obj->praid_follower.committed_praid_lot.topo_ctx);
			old_n_segs = XDLIST_N_ELEMNTS(&(praid_obj->praid_follower.committed_praid_lot.all_seg_lot_list));

			if (NVMEIBT_OBJ_IS_MARKED_OUTDATED(praid_obj)) {
				continue;
			}

			old_praid_size = sizeof(struct nvmeibt_praid_serialized_topo) + old_n_segs * sizeof(struct nvmeibt_serialized_seg_leader_topo);
	...
	if (dst_data_ptr) {
		...
		actual_written_size = (int)(*dst_data_ptr - (char *)output_header);
		NTOMA_ASSERT(topo_inc_sz_chk, actual_written_size == total_size,
					 "Size mismatch! calculated=@INT actual_written=@INT",
					 total_size, actual_written_size);

		output_header->topo_len = LE_SWAP32(total_size);
		...
		dst_wire_ctx->tlv_crc = 0;
		dst_wire_ctx->tlv_crc = crc32(0, dst_wire_ctx, sizeof(*dst_wire_ctx));
		dst_wire_ctx->tlv_crc = LE_SWAP32(crc32(dst_wire_ctx->tlv_crc, output_header, total_size));
	}
								   ...
								   char **dst_data_ptr, char **old_data_ptr, const char **upd_data_ptr)
{
	...
	case TLV_TYPE_TOPO_INCREMENTAL:
		total_size = merge_topo_incremental(dst_wire_ctx, old_wire_ctx, upd_wire_ctx,
											dst_data_ptr, old_data_ptr, upd_data_ptr,
											old_len, upd_len);
		break;
...
static int raft_leader_send_appendentries_to_a_peer(struct nvmeibt_raft_member *dst_member, int is_with_raft_log)
{
	...
	int64_t									peer_topo_idx;
	int64_t									peer_topo_config_idx;
	int64_t									peer_kafka_mgmt_config_offset;
	int64_t									peer_raft_members_kafka_offset;
	int64_t									peer_raft_members_seq_no;
	BOOL									is_peer_incremental_wire_buf_supported;
	BOOL									is_topo_incremental;
	...
	//
	// Compute incremental flags based on feature enable, peer's state, and incremental windows
	TODO(Merge incremental logic into compare_persist_and_wire_bufs_tlvs_excl_raft_ctx);
	peer_topo_idx = nvmeibt_tlv_get_idx(&(dst_member->committed_persist_and_wire_buf_hdr.topo_ctx));
	peer_topo_config_idx = nvmeibt_tlv_get_idx(&(dst_member->committed_persist_and_wire_buf_hdr.topo_config_ctx));
	...
	// the kafka_mgmt_config deletion guard implicitly forces complete topo_config as well.
	// The follower's merge_topo_config_incremental appends non-visited hash praids, so if
	// an incremental omitted a deleted praid, the follower would keep it — corrupting topology.
	...
	case PERSIST_AND_WIRE_BUF_DIFF_TOPO_AND_CONFIGS:
		...
		if (is_configs_and_raft_members_incremental) {
			NTOMA_ASSERT(non_topo_inc_assert, is_topo_incremental, "is_configs_incremental==true but is_topo_incremental==false -- impossible case");
			send_persist_and_wire_buf = my_raft_global.leader_to_commit_persist_and_wire_buf_with_conf_incremental;
		} else if (is_topo_incremental) {
			send_persist_and_wire_buf = my_raft_global.leader_to_commit_persist_and_wire_buf_topo_inc_configs_complete;
		} else {
...
static bool is_new_build_version_format(char *build_version)
{
	return ((build_version[1] == '.') || (build_version[2] == '.') || (build_version[3] == '.') || (build_version[4] == '.'));
}
...
static int dispatch_raft_msg(struct raft_msg *msg, struct nvmeibt_node *src_node)
{
	...
	if (src_member) {
		...
		if (is_new_build_version_format(msg->build_version) && strncmp(src_member->build_version, msg->build_version, sizeof(src_member->build_version) - 1)) {
...
void nvmeibt_raft_get_real_time_errors_str(struct nvmeibt_Str *out)
{
	...
	if (my_raft_global.n_persist_failures > 0){
		nvmeibt_Str_sprintf(out, "Err=7002, Cannot commit topology! num_errors=%d last %lld[sec] ago,\n",
	...
	if (nvmeibt_raft_is_leader()) {
		const int64_t		leader_topo_idx = extract_lower_32_bits_idx(RAFT_COMMIT_LIFECYCLE_VAL(TOPO, leader_to_commit));
		NVMEIB_HASH_FOREACH(peer_member, my_raft_global.raft_members_hash_by_uuid) {
			const int64_t	peer_topo_idx =   extract_lower_32_bits_idx(nvmeibt_tlv_get_idx(&(peer_member->committed_persist_and_wire_buf_hdr.topo_ctx)));
			const int64_t	topo_behind = leader_topo_idx - peer_topo_idx;
			if (topo_behind > 2)
				nvmeibt_Str_sprintf(out, "Err=7004, %s is %lld topologies behind leader,\n", nvmeibt_raft_member_name(peer_member), topo_behind);
		}
	} else if (my_raft_global.follower_to_commit_persist_and_wire_buf_full) {
		const int64_t		leader_topo_idx = extract_lower_32_bits_idx(nvmeibt_tlv_get_idx(&(my_raft_global.follower_to_commit_persist_and_wire_buf_full->topo_ctx)));
		const int64_t		my_topo_idx = extract_lower_32_bits_idx(RAFT_COMMIT_LIFECYCLE_VAL(TOPO, follower_committed));
		const int64_t		topo_behind = leader_topo_idx - my_topo_idx;
		if (topo_behind > 2)
			nvmeibt_Str_sprintf(out, "Err=7005, I am %lld topologies behind leader,\n", topo_behind);
	}
}
