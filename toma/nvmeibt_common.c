...
#define PADDED_MMAP_MAGIC_NUM 0x726f656568657265LLU
...
void init_padded_mmap_magic_number_struct(struct padded_mmap_magic_number *magic_num_struct,\
										  size_t length, void *addr)
{
	magic_num_struct->addr		= addr;
	magic_num_struct->length	= length;
	magic_num_struct->magic_num = PADDED_MMAP_MAGIC_NUM;
}

static inline bool is_addr_page_aligned(void *addr)
{
	return (((uintptr_t)addr % PAGE_SIZE) == 0);
}

static inline size_t padded_mmap_length(size_t length) //I want to call this "nvmeibt_mmap_length()"...
{
	return length + 2 * PAGE_SIZE;
}

static inline void *padded_mmap_aligned_addr(void* addr)
{
	return addr ? (void *) roundup((uintptr_t) addr, PAGE_SIZE) - PAGE_SIZE : NULL;
}
...
/* mmap wrapper that creates a protected page before and after the allocation.
   ...
   1. Allocating 2 pages more than length requested with no access permissions PROT_NONE
   2. Calling another mmap with the flags given to nvmeibt_mmap() to overide the last mmap (except for the first and last pages)
   3. Writing to the first page details about the allocation and setting it back to no access permissions
...
void *nvmeibt_mmap(size_t length, int fd)
{
	...
	/* We give mprotect() __len == 1 so we only modify permissions for a single
	   page. In case the struct is bigger than the size of one page we crash
	   immediatly after when we try to write to the 2nd page. */
	if (mprotect(mapped_padded, 1, PROT_WRITE) < 0) {
		mapped = MAP_FAILED;
	...
	mapped = mmap(mapped_padded + PAGE_SIZE, length,\
...
/* mmap wrapper that unmaps anything mapped using nvmeibt_mmap.
   This function verifies the arguments given (must be aligned and
   the data written to the first page must match)
   if everything matches then a simple munmap() is executed on the entire mmap
   range (including the 2 extra pages) */
int nvmeibt_munmap(void *addr, size_t length)
{
	int rv;
	struct padded_mmap_magic_number *magic_num_struct;
	void *mapped_padded;
	...
	mapped_padded = padded_mmap_aligned_addr(addr);
	length_padded = padded_mmap_length(length);

	mprotect(mapped_padded, 1, PROT_READ);
	magic_num_struct = mapped_padded;
