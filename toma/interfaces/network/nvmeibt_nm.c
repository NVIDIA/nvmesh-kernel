...
void nvmeibt_nm_mutex_lock(pthread_mutex_t *p)
{
    ...
    if ((pt_err = pthread_mutex_lock(p)) != 0) {
		N_ETf(nm_mutex_lock_e1, "Failed to lock - err=@INT (@STR)", pt_err, strerror(pt_err));
		abort();
    }
}
...
void nvmeibt_nm_mutex_release(pthread_mutex_t *p)
{
    ...
    if ((pt_err = pthread_mutex_unlock(p)) != 0) {
		N_ETf(nm_mutex_unlock_e1, "Failed to unlock - err=@INT (@STR)", pt_err, strerror(pt_err));
		abort();
    }
}
...
static int start_thread(struct nvmeibt_nm_local_node *ln)
{
	...
	if ((rv = pthread_attr_init(&attr)) != 0 ||
		...
		N_ETf(nm_start_thread_e1, "Fail to create ibud thread - err=@INT (@STR)", rv, strerror(rv));
		rv = -1;
	}
	else {
...
static struct nvmeibt_nm_local_node * create_local_node(struct nvmeibt_nm_hw_function_table *tbl)
{
	...
	if (pt_err != 0) {
		N_ETf(nm_create_local_node_e2, "Failed to create guard - err=@INT (@STR)", pt_err, strerror(pt_err));
		goto free_ln;
	}
	...
	ln->key_val_hash_by_u64_key = NVMEIB_HASH_CREATE(vtshgve, (HASH_MIN_LOG2_OF_N_ARR_ENTRIES + 4), "nm_key_val", 8, 0);
