...
/***** ACT_TOPO builder — shared by production serialize_active_topology() and the sandbox peer simulator ****/
void nvmeibt_act_topo_builder_init(struct nvmeibt_act_topo_builder *b, char *buf, int buf_size)
{
	b->buf = buf;
	b->buf_size = buf_size;
	b->n_segs = 0;
	b->topo_len = (int)sizeof(struct nvmeibt_active_topo_header);
	b->next_seg = (struct nvmeibt_serialized_seg_active_topo *)(buf + b->topo_len);
}

struct nvmeibt_serialized_seg_active_topo * nvmeibt_act_topo_builder_append(struct nvmeibt_act_topo_builder *b)
{
	int needed = b->topo_len + (int)sizeof(struct nvmeibt_serialized_seg_active_topo);
	struct nvmeibt_serialized_seg_active_topo *seg = b->next_seg;
...
void nvmeibt_act_topo_builder_to_wire(const struct nvmeibt_act_topo_builder *b)
{
	struct nvmeibt_active_topo_header *header_ptr =      (typeof(header_ptr))b->buf;
	struct nvmeibt_serialized_seg_active_topo *seg_ptr = (typeof(seg_ptr))(header_ptr + 1);
	int i;
	...
	header_ptr->res = 0;
	for (i = 0; i < b->n_segs; i++)	// Convert segments to wire byte order
		nvmeibt_disk_segment_convert_active_bin_topo_le_be(&seg_ptr[i]);
	nvmeibt_topology_convert_follower_header_le_be(header_ptr);	// Convert header to wire byte order
}
